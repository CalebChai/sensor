...
def draw(raw_data, calib_data, * , axis = None):
    ...
    if axis is not None:
        ...
        for i,index in enumerate(axis):
            # random a color
            axs[i].plot(raw_data[:,index], 'r', label=f"raw{index}")
            axs[i].plot(calib_data[:,index], 'b', label=f"calib{index}")
    else:
        fig, ax = plt.subplots()
...
def plot_matrix(matrix):
    import matplotlib.pyplot as plt
    print("Matrix values:\n", matrix)  # Print the matrix values
    fig, ax = plt.subplots()
...
plot_matrix(model.linear.weight.detach().cpu().numpy())
